...
def transmit():
    ...
    try:
        ...
        # Send the target coordinates upstream over the RFD-900x radio link.
        # This injects a DEBUG_VECT MAVLink message into the local MAVProxy router on UDP 14551.
        try:
            from pymavlink import mavutil
            # udpout sends packets directly to the MAVProxy listener
            mav = mavutil.mavlink_connection('udpout:127.0.0.1:14551')
            # Pack lat, lon, and spread into the x, y, z floats of DEBUG_VECT
            mav.mav.debug_vect_send(b'KRAKEN_TGT', int(time.time()*1e6), lat, lon, spread_m)
            logger.info(f"Target sent UPSTREAM over RFD-900x MAVLink: {lat}, {lon}")
        except ImportError:
